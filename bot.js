...
const WIND_THRESHOLD_MS  = 5.5;
const ALERT_DURATION_MIN = 30;
...
const WIND_KEYS = [
  ...
  "ws", "WS", "Ws", "meanWindSpeed", "mean_wind_speed",
  "avgWindSpeed", "avg_wind_speed", "windVelocity", "wind_velocity",
  "speed", "Speed",
];
...
// =============================================
// Получение данных (с расширенными заголовками)
// =============================================
...
async function fetchWindSpeed() {
  ...
  const res = await fetch(WEATHER_URL, {
    ...
    headers: {
      ...
      "Referer": "https://dubaiweather.dm.ae/",
      "Origin": "https://dubaiweather.dm.ae",
  ...
  console.log(`[${nowStr()}] Ответ API (первые 300 символов): ${text.slice(0, 300)}`);

  let data;
  try {
    data = JSON.parse(text);
  } catch {
    throw new Error(`Ответ не является JSON: ${text.slice(0, 200)}`);
  }

  let speed = findWindSpeed(data);
  if (speed === null) throw new Error(`Поле wind speed не найдено. Ключи: ${Object.keys(data).join(", ")}`);
