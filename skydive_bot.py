...
def calc_aff_prob(fl):
    ...
    for item in fl[:4]:
        s = 100
        # Облачность — жёсткое ограничение 30%
        c = item["clouds"]["all"]
        if c >= 30: s -= 60
        elif c >= 20: s -= 20
        elif c >= 10: s -= 10
        # Осадки — запрет
        if "rain" in item or "snow" in item: s = 0
        elif item["weather"][0]["id"] < 700: s = 0
        else:
            # Ветер — жёсткое ограничение 7м/с
            w = item["wind"]["speed"]
            if w > 7: s = 0
            elif w > 5: s -= 40
            elif w > 3: s -= 15
            # Порывы — жёсткое ограничение 7м/с
            g = item["wind"].get("gust", 0)
            if g > 7: s = 0
            elif g > 5: s -= 30
            # Видимость
...
async def make_message(label):
    ...
    for item in fl[:4]:
        ...
        lines.append(f"  {t} | {round(item['main']['temp'])}°C | 💨{w}м/с (пор.{g:.1f}) | ☁️{c}% | {d}")
    return (
        f"🪂 *Прогноз — {label}*\n📍 Крутицы, Рязанская обл.\n\n"
        f"{emoji(prob)} *Опытные: {prob}%*\n"
        f"{emoji(aff_prob)} *AFF студенты: {aff_prob}%*\n"
        f"_(ветер/порывы ≤7м/с, облачность <30%)_\n\n"
            ...
            for update in data.get("result", []):
                ...
                if text == "/start":
                    await tg_send(
                        ...
                        "Показывает вероятность для опытных и AFF студентов отдельно 🎓"
                    )
                elif text == "/weather":
