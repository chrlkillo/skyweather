...
async def get_weather():
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get("https://api.open-meteo.com/v1/forecast", params={
            ...
            "pressure_level_variables": "windspeed,winddirection",
            "pressure_level": "700,800,850,925",
            "models": "ecmwf_ifs",
...
def _parse_open_meteo(raw):
    ...
    for i, t in enumerate(times):
        ...
        # Pressure level winds: 925hPa~750м, 850hPa~1500м, 800hPa~2000м, 700hPa~3000м
        w925 = hourly.get("windspeed_925hPa", [0]*len(times))[i] or 0
        d925 = hourly.get("winddirection_925hPa", [0]*len(times))[i] or 0
        w850 = hourly.get("windspeed_850hPa", [0]*len(times))[i] or 0
        d850 = hourly.get("winddirection_850hPa", [0]*len(times))[i] or 0
        w800 = hourly.get("windspeed_800hPa", [0]*len(times))[i] or 0
        d800 = hourly.get("winddirection_800hPa", [0]*len(times))[i] or 0
        w700 = hourly.get("windspeed_700hPa", [0]*len(times))[i] or 0
        d700 = hourly.get("winddirection_700hPa", [0]*len(times))[i] or 0
        item = {
            ...
            "wind": {
                ...
                "gust": round(gust, 1),
                "deg": round(dir10),
                "deg_120m": round(dir120),
                # Высотные ветры (м/с, градусы)
                "w925": round(w925, 1), "d925": round(d925),   # ~750м
                "w850": round(w850, 1), "d850": round(d850),   # ~1500м
                "w800": round(w800, 1), "d800": round(d800),   # ~2000м
...
def calc_jumprun(items_now):
    """Рассчитывает снос и точку выброски для текущего часа."""
    if not items_now:
        return None
    item = items_now[0]
    w = item["wind"]
    ...
    # Высоты и ветра
    # Свободное падение: 4000м → 1000м (~60 сек), купол: 1000м → 0м (~240 сек)
    layers = [
        {"label": "4000м", "speed": w.get("w700", 0), "dir": w.get("d700", 0), "time_ff": 15},   # 4000-3000м, ~15с
        {"label": "3000м", "speed": w.get("w700", 0), "dir": w.get("d700", 0), "time_ff": 15},   # 3000-2000м, ~15с
        {"label": "2000м", "speed": w.get("w800", 0), "dir": w.get("d800", 0), "time_ff": 15},   # 2000-1000м, ~15с
        {"label": "1000м", "speed": w.get("w925", 0), "dir": w.get("d925", 0), "time_ff": 15},   # 1000м — раскрытие
        {"label": "300м",  "speed": w["speed_100m"],  "dir": w.get("deg_120m", 0), "time_cp": 60},  # купол верх
        {"label": "Земля", "speed": w["speed"],        "dir": w.get("deg", 0),     "time_cp": 180}, # купол низ
    ]

    import math

    # Снос в свободном падении (4000→1000м, ~60с)
    ff_x, ff_y = 0.0, 0.0
    for layer in layers[:4]:
        ...
        ff_x += spd * math.sin(rad) * t
        ff_y += spd * math.cos(rad) * t

    # Снос под куполом (1000→0м, ~240с)
    cp_x, cp_y = 0.0, 0.0
    for layer in layers[4:]:
        spd = layer["speed"]
        deg = layer["dir"]
        t = layer["time_cp"]
        rad = math.radians(deg)
        cp_x += spd * math.sin(rad) * t
        cp_y += spd * math.cos(rad) * t

    total_x = ff_x + cp_x
    total_y = ff_y + cp_y
    total_drift = round(math.sqrt(total_x**2 + total_y**2))

    # Направление сноса (куда сносит)
    drift_deg = round(math.degrees(math.atan2(total_x, total_y)) % 360)

    # Точка выброски — в обратном направлении от сноса
    exit_deg = (drift_deg + 180) % 360

    # Рекомендации по отводу
    if total_drift < 300:
        recommendation = "0м — выброска над точкой"
    elif total_drift < 600:
        recommendation = f"500м до точки (курс {exit_deg}°)"
    elif total_drift < 900:
        recommendation = f"700м до точки (курс {exit_deg}°)"
    elif total_drift < 1200:
        recommendation = f"1000м до точки (курс {exit_deg}°)"
    else:
        recommendation = f"1500м до точки (курс {exit_deg}°)"

    return {
        "layers": layers,
        "ff_drift": round(math.sqrt(ff_x**2 + ff_y**2)),
        "cp_drift": round(math.sqrt(cp_x**2 + cp_y**2)),
...
def format_jumprun(jr, today_fl):
    """Форматирует сообщение /jumprun."""
    if jr is None:
        return "⚠️ Нет данных для расчёта."

    w = jr["item"]["wind"]
    layers_info = [
        ("4000м", w.get("w700", 0), w.get("d700", 0)),
        ("3000м", w.get("w700", 0), w.get("d700", 0)),
        ("2000м", w.get("w800", 0), w.get("d800", 0)),
        ("1000м", w.get("w925", 0), w.get("d925", 0)),
        (" 300м", w["speed_100m"],  w.get("deg_120m", 0)),
        ("Земля", w["speed"],       w.get("deg", 0)),
    ]

    wind_lines = "\n".join(
        f"  {lbl}: {spd:.1f} м/с  {deg}°"
        for lbl, spd, deg in layers_info
    )

    # Облачность по часам сегодня
    cloud_lines = []
    ...
    return (
        ...
        f"💨 Ветер по высотам:\n{wind_lines}\n\n"
        f"📐 Снос:\n"
        f"  Свободное падение (4000→1000м): {jr['ff_drift']}м\n"
        f"  Купол (1000→0м): {jr['cp_drift']}м\n"
        f"  Итого: {jr['total_drift']}м  направление {jr['drift_deg']}°\n\n"
        f"🎯 Точка выброски: {jr['recommendation']}\n\n"
        f"☁️ Облачность сегодня:\n{cloud_section}"
    )
