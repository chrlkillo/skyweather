...
async def get_weather():
    """Получает погоду с Open-Meteo (те же данные что в Windy.com — ECMWF/GFS модели)."""
    async with httpx.AsyncClient() as client:
        r = await client.get("https://api.open-meteo.com/v1/forecast", params={
            "latitude": LAT,
            "longitude": LON,
            "hourly": "temperature_2m,apparent_temperature,precipitation_probability,precipitation,weathercode,cloudcover,cloudcover_low,cloudcover_mid,windspeed_10m,windgusts_10m,winddirection_10m,visibility",
            "wind_speed_unit": "ms",
...
def _parse_open_meteo(raw):
    """Конвертирует ответ Open-Meteo в унифицированный формат."""
    hourly = raw["hourly"]
    times = hourly["time"]
    items = []
    for i, t in enumerate(times):
        ...
        wind = hourly["windspeed_10m"][i] or 0
        gust = hourly["windgusts_10m"][i] or wind
        clouds_low = hourly["cloudcover_low"][i] or 0
        clouds_mid = hourly["cloudcover_mid"][i] or 0
        clouds = max(clouds_low, clouds_mid)  # облачность до ~5000м
        vis = (hourly["visibility"][i] or 10000)
        precip_prob = hourly["precipitation_probability"][i] or 0
        has_precip = _is_precipitation(code)
        item = {
            "dt": int(dt_msk.timestamp()),
            "main": {
                "temp": hourly["temperature_2m"][i] or 0,
                "feels_like": hourly["apparent_temperature"][i] or 0,
                "humidity": 0,
            },
            "wind": {
                "speed": round(wind, 1),
                "gust": round(gust, 1),
                "deg": hourly["winddirection_10m"][i] or 0,
            },
            "clouds": {"all": int(clouds)},
            "visibility": int(vis),
            "weather": [{"id": 500 if has_precip else 800, "description": _wmo_description(code)}],
            "rain": {} if has_precip and code < 70 else None,
            "snow": {} if has_precip and code >= 70 else None,
...
async def make_message(label):
    ...
    def hour_lines(items):
        ...
        for item in items:
            ...
            result.append(f"{ind} {t} | {round(item['main']['temp'])}°C | 💨{w}м/с (пор.{g:.1f}) | ☁️{c}% | {d}")
        return result
