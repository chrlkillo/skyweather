...
async def get_weather_raw():
    """Возвращает сырой ответ API."""
    async with httpx.AsyncClient() as client:
        r = await client.get("https://api.open-meteo.com/v1/forecast", params={
            "latitude": LAT,
            "longitude": LON,
            "hourly": "wind_speed_600hPa,wind_direction_600hPa,wind_speed_700hPa,wind_direction_700hPa,wind_speed_800hPa,wind_direction_800hPa,wind_speed_850hPa,wind_direction_850hPa,wind_speed_900hPa,wind_direction_900hPa,wind_speed_925hPa,wind_direction_925hPa,wind_speed_975hPa,wind_direction_975hPa",
            "models": "ecmwf_ifs",
        ...
        try:
            ...
            for update in data.get("result", []):
                ...
                if text == "/start":
                    ...
                    try:
                        data = await get_weather()
                        hourly = data["list"][0]["wind"] if data["list"] else {}
                        raw = await get_weather_raw()
                        keys = list(raw.get("hourly", {}).keys())
                        sample = {k: raw["hourly"][k][12] for k in keys if "600" in k or "700" in k or "800" in k or "900" in k or "925" in k or "975" in k}
                        await tg_send(f"Keys: {keys}\n\nSample i=12:\n{sample}", chat_id=user_chat_id)
                    except Exception as e:
