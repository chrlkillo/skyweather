...
def calculate_skydive_probability(forecast_list):
    ...
    for item in forecast_list[:4]:
        ...
        if clouds > 80: score -= 40
        elif clouds > 50: score -= 20
        elif clouds > 30: score -= 10
        if "rain" in item or "snow" in item: score -= 50
        if item["weather"][0]["id"] < 700: score -= 40
        wind_speed = item["wind"]["speed"]
        if wind_speed > 10: score -= 40
        elif wind_speed > 7: score -= 25
        elif wind_speed > 5: score -= 10
        wind_gust = item["wind"].get("gust", 0)
        if wind_gust > 12: score -= 20
        elif wind_gust > 8: score -= 10
        visibility = item.get("visibility", 10000)
        if visibility < 3000: score -= 30
        elif visibility < 5000: score -= 15
...
async def send_forecast(bot: Bot, period_label: str):
    try:
        ...
        await bot.send_message(chat_id=CHAT_ID, text=f"⚠️ Ошибка: {e}")
