...
async def make_message(label):
    ...
    def make_3h_blocks(items):
        """Группирует часы в 3-часовые отрезки и считает вероятность каждого."""
        # Группируем по 3 часа: 08-11, 11-14, 14-17, 17-20
        slots = {}
        for item in items:
            dt_msk = datetime.fromtimestamp(item["dt"], tz=MSK)
            h = dt_msk.hour
            # Начало отрезка: 8, 11, 14, 17
            slot_start = (h // 3) * 3
            if slot_start < DAYLIGHT_START:
                slot_start = DAYLIGHT_START
        ...
        for slot_start in sorted(slots.keys()):
            ...
            lines.append(
                ...
                f"💨{w_avg}↕{w100_avg}м/с (пор.{g_avg}) | ☁️{c_avg}% | {d}\n"
        ...
        # Вероятность дня = доля хороших отрезков
    ...
    def day_section(title, prob, aff, items):
        ...
        s = f"*{title}:*\n"
        s += f"{emoji(prob)} Опытные: *{prob_str(prob)}*\n"
        s += f"{day_ind} AFF студенты: *{day_aff_pct}%* ({sum(1 for b in blocks if 'AFF: 0%' not in b)}/{total} отрезков)\n"
        s += "\n".join(blocks) + "\n"
        return s
